...
**Phase A — JPA schema.** New `song_library_folders`/`song_library_files` tables (`V3` migration, both dialects), `SINGLE_TABLE`-inheritance-shaped for the `FolderEntity`/`AbstractFileEntity` hierarchies. Rather than retrofitting `@Entity` annotations onto the domain classes directly (`RootFolderEntity` etc. have `equals`/`hashCode`/`compareTo` rooted in a filesystem-path-shaped `getNaturalIdentity()`, and hold children in natural-ordering `TreeSet`s — a bad fit for Hibernate-managed bidirectional collections), the JPA layer uses flat, standalone row types (`SongLibraryFolderJpaEntity`/`SongLibraryFileJpaEntity`) and `SongLibraryRepositoryJpaImpl` manually assembles/decomposes the `RootFolderEntity` tree — the same "flat rows + manual aggregate assembly" shape `UserRepositoryJpaImpl` already used for `UserEntity`. `SongLibraryRepositoryPostgresImpl` (a stub) was deleted; `song-library.repository-type` renamed `postgres` → `jpa`.
