...
- **Local walk-up credits**: physical patrons keep paying via the bill acceptor exactly as today (`JukeANatorFrame`'s local `CreditManager`, a Swing-UI-only, in-memory counter, entirely separate from `UserService`'s web credit balance). Queueing songs with these credits is unmodified by this feature. *Update, see `docs/financial-ledger-refactor-and-sync.md`:* the underlying *financial* record of a local cash/credit-card payment (`FinancialLedgerService.recordLocalCashCredit`/`recordLocalCreditCardCredit`) is now mirrored to master, best-effort, so a central DBA/bar-owner sees all revenue for a location in one place (`location_transaction`, filterable by `location_id`) rather than only in each slave's own local database.
...
- **Repository swap pattern**: every domain (`song-library`, `song-queue`, `user`) has a repository interface with `filesystem`/`jpa` impls selected via `@ConditionalOnProperty(name="X.repository-type", havingValue=...)` in [AppConfig.java](src/main/java/com/djt/jukeanator_engine/config/AppConfig.java). New `location` and `credit-ledger` domains follow the same shape.
...
3. No Flyway migrations exist anywhere despite the pom dependency — first real JPA impl (credit ledger) needs a baseline migration written from scratch.
...
# JPA-backed song library + locationId-everywhere refactor

**Status: Implemented.** Verified via `mvn clean compile`/`mvn test-compile`/full `mvn test` after every phase — see the per-phase notes below. This directly supersedes the "Not in scope (explicitly deferred)" line at the end of the previous section: `SongIdentifier` now carries a `locationId`, every relevant service method takes one as its first parameter, and the `*LocationProxy`/`LocationScoped*Controller` class family described throughout the plan above is gone.

## Context

The design above shipped a real, working master/slave system, but it took a "snapshot + proxy" shape: master served song-library browse reads from a synced JSON snapshot on disk via `SongLibraryServiceLocationProxy` (and the analogous queue/player proxies), through a parallel `LocationScoped*Controller` family kept deliberately separate from the standalone/slave controllers. This refactor replaces that with `SongLibraryRepository` gaining a real JPA/MySQL implementation — one database table set holding every location's songs, tenant-separated by `locationId` — and collapses the proxy/scoped-controller duplication: every relevant method now takes `locationId` directly, and standalone mode is simply "multi-tenant with one location" rather than a structurally different code path. This was a deliberate, confirmed breaking change: existing persisted playlists/song-identifiers were treated as obsolete (no migration), since there was no real production master deployment yet.

## What changed, by phase

**Phase A — JPA schema.** New `song_library_folders`/`song_library_files` tables (`V3` migration), `SINGLE_TABLE`-inheritance-shaped for the `FolderEntity`/`AbstractFileEntity` hierarchies. Rather than retrofitting `@Entity` annotations onto the domain classes directly (`RootFolderEntity` etc. have `equals`/`hashCode`/`compareTo` rooted in a filesystem-path-shaped `getNaturalIdentity()`, and hold children in natural-ordering `TreeSet`s — a bad fit for Hibernate-managed bidirectional collections), the JPA layer uses flat, standalone row types (`SongLibraryFolderJpaEntity`/`SongLibraryFileJpaEntity`) and `SongLibraryRepositoryJpaImpl` manually assembles/decomposes the `RootFolderEntity` tree — the same "flat rows + manual aggregate assembly" shape `UserRepositoryJpaImpl` already used for `UserEntity`. The old single-vendor stub repository implementation was deleted; `song-library.repository-type` was renamed to `jpa`.

**Phase B — `SongLibraryServiceImpl`/`SongLibraryController` unification.** `SongLibraryServiceImpl` now holds `Map<Integer, RootFolderEntity> songLibraryRoots` (loaded lazily per locationId) instead of a single root; every browse method takes `Integer locationId` first. `SongLibraryController` moved to `/api/locations/{locationId}/song-library/**`. Deleted: `SongLibraryServiceLocationProxy`, `LocationScopedSongLibraryController`.

**Phase C — `SongQueueServiceImpl`/`SongQueueController` unification.** Every method branches internally: `isOwnLocation(locationId)` ? run the existing local logic : forward via the existing `SlaveCommandGateway`. This is the one class genuinely safe to merge this way — its local state is plain in-memory data structures, no hardware side effects — so master now constructs a real `SongQueueServiceImpl` too (previously excluded via `NotMasterModeCondition`), paired with a new `NoOpBackgroundMusicService` master-only bean. Deleted: `SongQueueServiceLocationProxy`, `LocationScopedSongQueueController`.

**Phase D — `SongPlayerServiceImpl`/`SongPlayerController` unification, with one deliberate exception to the merge pattern.** `SongPlayerServiceImpl`'s constructor has real side effects (spins up a VLC/Winamp process, starts a continuously-running watchdog thread) that must never happen on a headless master host, so — unlike the queue case — it stays a **separate class** from a new `SongPlayerServiceMasterImpl` (pure `SlaveCommandGateway` forwarding, no hardware). Both implement the same `SongPlayerService` interface and are selected via the same `@ConditionalOnProperty`/`NotMasterModeCondition` bean-selection pattern already used for repository-type switches elsewhere, both registered under the bean name `songPlayerService` so `SongPlayerController`'s `@Qualifier` resolves either one. `SongPlayerController` moved to `/api/locations/{locationId}/song-player/**`. Deleted: `SongPlayerServiceLocationProxy`, `LocationScopedSongPlayerController`, and (once song-player's proxy was gone too) `LocationServiceRegistry` itself, since nothing used it anymore.

**Phase E — `SongIdentifier` gains `locationId`.** New `V4` migration adds `location_id` to `user_song_play_history`/`playlist_songs` (clean-slate `DELETE FROM` on both, per the confirmed breaking-change decision). `UserEntity.addSongToPlaylist`/`removeSongFromPlaylist`, `UserService`, and `UserController`'s four playlist-song endpoints all thread `locationId` through explicitly now, rather than assuming "always this instance's own location."

**Phase F — slave→master sync populates JPA.** `LocationServiceImpl.receiveLibraryMetadataSync()` still writes the JSON snapshot file unconditionally (it remains the source of truth for cover-art-hash diffing — `coverArtHash` has no analog in the JPA schema), but when `song-library.repository-type=jpa` it additionally builds a `RootFolderEntity` from the synced `LibrarySnapshotDto` and stores it via the JPA repository, so `SongLibraryServiceImpl` transparently serves that location's browse reads from the database from then on — no proxy, no live round-trip to the slave. `LibrarySyncService`'s scan-triggered sync now runs on a background thread instead of inline on the scan's own thread.

## Known gaps / follow-ups

- **Compilation-album artists** that only exist as a song-embedded artist name (no real `ArtistFolderEntity` folder — see `ArtistFromSongEntity`) are not round-tripped through the new JPA schema; `RootFolderEntity.artistsFromSongs` is populated by `SongScanner` at scan time, which a JPA-hydrated root never runs. Artist search for these may be incomplete on a JPA-backed master until a follow-up extends the schema.
- **No live JPA verification**: there was no Docker/local MySQL available while this was built, so `SongLibraryRepositoryJpaImpl`'s actual round-trip (store → reload → confirm every field matches) has not been exercised against a real database — only unit/mock-level and filesystem-repository-type tests ran. Run it, and the Testcontainers-based `MySql*ApplicationTests`, wherever Docker becomes available.
- `SongScanner`'s genre/artist scan-local id counters were not deeply audited against the new `UNIQUE (location_id, folder_type, source_id)` constraint — worth a close look before relying on this in production.