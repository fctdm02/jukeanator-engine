...
public class CalendarAligner implements TemporalAdjuster {
  private static final List<TemporalField> orderedFields = Arrays.asList(ChronoField.NANO_OF_SECOND,
      ChronoField.MICRO_OF_SECOND, ChronoField.MILLI_OF_SECOND, ChronoField.SECOND_OF_MINUTE,
      ChronoField.MINUTE_OF_HOUR, ChronoField.HOUR_OF_DAY, ChronoField.DAY_OF_MONTH,
  ...
  CalendarAligner(long interval, TemporalField field, CalendarAlignmentStrategy strategy) {
  ...
  private static int validateConstructorParams(long interval, TemporalField field,
    ...
    orderedFields.stream().filter(f -> f == field).findAny()
    ...
    if (field != ChronoField.YEAR) {
      /*
       * checkArgument( (field.range().getMaximum() - field.range().getMinimum() + 1) % interval ==
       * 0, "The interval must be evenly divisible into the range of the field");
       */
    }
    return IntStream.range(0, orderedFields.size()).filter(i -> orderedFields.get(i) == field)
        .findFirst().getAsInt();
